...
public interface StoryRepository extends JpaRepository<Story, Integer> {
    ...
    @Query("SELECT s FROM Story s WHERE s.endAt > :currentTime")
    ...
    List<Story> findMyActiveStoriesByUserId(@Param("userId") Integer userId);

    @Query("SELECT s " +
            "FROM Story s " +
            "WHERE s.userId = :landOwnerId " +
            "AND s.city = :city " +
            "AND s.cityDetail = :cityDetail " +
            "AND s.endAt > CURRENT_TIMESTAMP " +
            "ORDER BY s.createdAt ASC")
    List<Story> findActiveStoriesByLandOwnerId(@Param("landOwnerId") Integer landOwnerId,
